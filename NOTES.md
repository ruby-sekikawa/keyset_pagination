...
### ★ 追加検証: 「裾は最初の1回だけでは？」への回答（重要）
問い: OFFSET の max 6527ms は、1万回叩いても最初のコールド1回だけでは？

切り分けの結論:
- **max 6527ms は YES = ほぼ一回限りの外れ値**（コールド/GC/競合）。2回目実行では max 126ms。
  → だから **max は指標に使わない**（単発ノイズに振り回される）。これが p95/p99 を見る理由。
- **p95≈77ms / p99≈100ms は NO = 叩くたび毎回起きる本物の裾**。2回の実行で p95=79/75, p99=103/99 と安定再現。

固定 offset を uncached(ARクエリキャッシュ無効)で5回ずつ warm 計測:

| offset | 5回の実測 |
|---|---|
| 20(浅) | 1.1, 0.9, 1.0, 1.0, 1.0 ms ← 毎回速い |
| 200000(深) | 74.0, 76.0, 76.3, 79.0, 77.9 ms ← **毎回遅い** |

EXPLAIN が示す理由（OFFSET 200000, idx_c あり）:
- `Index Scan using idx_c` / `Buffers: shared hit=201012`（全部hit=キャッシュ済）/ Execution 89ms。
- **OFFSET N は index を使っても N件のエントリを毎回たどって捨てる**。cost ∝ N。
- Buffers が全部 hit なのに 77ms = **I/O でなく CPU bound**（20万件を舐める計算）。
  → キャッシュが温まっても消えない。「深いページは構造的に毎回遅い」。
- 本番では裾が出るかは「時間(warmup)」でなく「どのページを引くか(入力)」で決まる。
  トラフィックの5%が深いページなら、その5%は毎回遅い（同じ深いページは何度開いても遅い）。
- keyset は深いページでも Buffers 24・数ms（境界へ直行し「歩いて捨てる」作業が無い）。

測定の落とし穴（自戒）: 最初 runner で「深いoffsetも0.8ms」と誤測。原因は AR クエリキャッシュ
（同一SQL反復で2回目以降キャッシュヒット＝DB未実行）。`ActiveRecord::Base.uncached` で正しく測り直した。
